...
Dependencies (install via project env, e.g. `uv sync` in this repo): `jax`, `jaxlib`,
`keras`, `keras-hub`, `ucimlrepo`, `numpy`, `pandas`, `matplotlib`.

JAX must be selected before importing Keras; see
...
def plot_lorenz_analysis(y_true, y_pred):
    ...
    if os.environ.get("MPLBACKEND") == "Agg":
        plt.savefig("customer_ltv_lorenz.png", dpi=120, bbox_inches="tight")
    else:
        plt.show()
